...
const MatFrBT = ({type}) => {
    ...
    useEffect(() => {
        (
            async () => {
                ...
                data4.forEach(pp => {
                    data5.filter(oo => oo.domainId == pp.id ).forEach(ac => {
                        const note = data6.filter(n => n.activitieId == ac.id).length > 0 ? parseFloat(data6.filter(n => n.activitieId == ac.id)[0].value) : 0;
                        if (ac.appreciationsNber == 4 && ( note != 2 || note != 1 )) {
                            ba.push(ac.name)
                        }
                        else if(ac.appreciationsNber == 3 && note !== 1){
                            ba.push(ac.name)
                        }
                    })
                })
    ...
    <nav className="navbar navbar-expand-lg" style={{padding: '10px 10px'}}>
        <a target={'_blank'} href={`http://localhost:4000/download/pdf/bul2/${class_id}/${student_id}/${exam_id}`} className="btn btn-success">Télécharger le bulletin</a>
    </nav>
        ...
        <table className='table table-bordered table-light'>
            <thead>
                <tr>
                    ...
                    <th colSpan={3}>GRADING OF COMPETENCE</th>
                </tr>
                <tr>
                    <th>Expert A+</th>
                    <th>Acquired A</th>
