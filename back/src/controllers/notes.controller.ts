module.exports.addOrUpdateNotes = async (req, res) => {
    ...
    req.connection.query('SELECT is_editable FROM settings WHERE id = 1', (errrrrrrr, respppppp) => {
		...
		if (is_editable === 'yes') {
			if ( value && exam_id && exam_id !== '' && student_id && student_id !== '' && class_id && class_id !== '' && matiere_id && matiere_id !== '' && tag_name && tag_name !== ''  ) {
				req.connection.query('SELECT * FROM matiere WHERE id = ?', [matiere_id], (error, respon) => {
					...
					if (parseFloat(value) <= over) {
					 req.connection.query('SELECT * FROM notes WHERE exam_id = ? AND student_id = ? AND matiere_id = ? AND tag_name = ?', [exam_id, student_id, matiere_id, tag_name], (err, resp) => {
						 if (resp.length < 1) {
							req.connection.query('INSERT INTO notes(student_id, exam_id, class_id, matiere_id, tag_name, value) VALUES(?, ?, ?, ?, ?, ?)', [student_id, exam_id, class_id, matiere_id, tag_name, val], (err2, resp2) => {
							   if(!err2){
							...
							req.connection.query('UPDATE notes SET student_id = ?, exam_id = ?, class_id = ?, matiere_id = ?, tag_name = ?, value = ? WHERE exam_id = ? AND student_id = ? AND matiere_id = ? AND tag_name = ?', [student_id, exam_id, class_id, matiere_id, tag_name, val, exam_id, student_id, matiere_id, tag_name], (err2, resp) => {
							   if(!err2){
								   req.connection.query('SELECT * FROM notes WHERE exam_id = ? AND student_id = ? AND class_id = ?', [exam_id, student_id, class_id], (err3, notes) => {
									   let totalPoints = 0;
									   notes.forEach(note => {
